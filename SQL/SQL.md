...
-- перевод данных из str к числу с точкой
...
-- если конвертация невозможна вернёт NULL вместо сконвертированного значения.
```
| Тип данных  | Описание |
| --------- | --------------------------------------------- |
...
| DECIMAL[(M[,D])] |	конвертирует значение в DECIMAL, имеет два необязательных аргумента M и D, определяющих максимальное количество цифр и количество цифр после запятой. По-умолчанию, D - 0, M - 10 |
| CHAR[(N)]	| конвертирует значение в TIME, в качестве необязательного аргумента можно передать максимальную длину строки |
| SIGNED	| конвертирует значение в BIGINT |
|UNSIGNED	| конвертирует значение в беззнаковое значение BIGINT |
| BINARY |	конвертирует значение вBINARY |
...
-- первым аргументом в одинарных кавычках поставлен пробел, через который соединяются значения столбцов
LOWER()
UPPER()
...
| Функция   | Описание работы                                      | Пример запроса    | Вывод |
| --------- | --------------------------------------------- | -------------- | -------------- |
| CEILING(x) | округляет x до целого числа в большую сторону | SELECT CEILING(2.2)<br>SELECT CEILING(-3.8) | 3<br>-3 |
| ROUND(x, k) | округляет x до k знаков после запятой, если k не указано – до целого | SELECT ROUND(8.3631)<br>SELECT ROUND(2.74562, 1) | 8<br>2.7 |
| GREATEST(num [,num]) | возвращает максимальное значение из списка   | GREATEST(1,2,-1) | 2 |
| LEAST(num [,num])  | возвращает минимальное значение из списка   | SELECT LEAST(1,2,-1) | -1 |
| SIGN([num]) | Возвращает знак переданного числа: -1 для отрицательных значений, 1 для положительных и 0 для 0   | SELECT SIGN(-69)<br>SELECT SIGN(0)<br>SELECT SIGN(69)| -1<br>0<br>1 |
| FLOOR(x)  | округляет до целого числа в меньшую сторону | SELECT FLOOR(9.3)<br>SELECT FLOOR(-7.8) | 9<br>-8 |
| POWER(x, y) | возведение x в степень y                        | SELECT POWER(2, 3) | 8.0 |
| SQRT(x)   | квадратный корень из x                          | SELECT SQRT(16)<br>SELECT SQRT(9) | 4.0<br>3.0. |
| DEGREES(x) | конвертирует x из радиан в градусы     | SELECT DEGREES(20) | 1145.91559026164 |
| RADIANS(x) | конвертирует x из градусов в радианы  | SELECT RADIANS(45) | 0.7853981633974483 |
| ABS(x)    | модуль числа x                                 | SELECT ABS(-5)<br>SELECT ABS(5) | 5<br>5 |
| PI()      | выводит число pi                            | SELECT PI() | 3.141592653589793 |
| POW(num, power)      | вычисляет число в указанной степени         | SELECT POW(4,5) | 1024 |
| LOG(base, num)      | вычисляет логарифм числа по указанному основанию       | SELECT LOG(10, 2) | 0,3010299957 |
| EXP(num)    | вычисляет экспоненциальное значение числа     | SELECT EXP(1) | 2,7182818285 |
| SIN(num)   | вычисляет синус числа    | SELECT SIN(3) | 0.1411200081 |
| COS(num)   | вычисляет косинус числа    | SELECT COS(0.5) | 0.8775825619 |
| TAN(num)   | вычисляет тангенс числа    | SELECT TAN(10) | 0.6483608275 |
...
| --------- | --------------------------------------------- |
| | |
| DATE | задает дату в формате YYYY-MM-DD |
| DATETIME, TIMESTAMP | задает дату в формате YYYY-MM-DD hh:mm:ss |
| TIME | задает время в формате hhh:mm:sss |
| YEAR | задает год в формате YYYY - полный формат, YY или Y  |
| STR_TO_DATE('Date: 31 December 2023', 'Date: %d %M %Y') | принимает произвольную строку, содержащую дату, и формат, описывающий её |
...
| MONTHNAME() | извлекает название месяца |
| NOW() | возвращает текущую дату и время сервера в формате datetime (год, месяц, день, часы, минуты, секунды) |
| DATEPART(year, column) -- возвращает значение указанной части даты и времени (год, месяц, день, час, минута или секунда) |
| DATEDIFF(day, startdate, enddate) | вычисляет разницу между двумя датами в заданной единице измерения |
...
| DATEADD(day, 10, column) |  добавляет заданное количество времени к дате в заданной единице измерения |
| EXTRACT(DOW FROM column) | возвращает дни недели из значений в формате TIMESTAMP (в PostgreSQL дни недели представлены числами от 0 до 6, где 0 - воскресенье, 1 - понедельник) |
| AGE(column, column)::VARCHAR | возвращает разницу между двумя значениями, представленными в формате TIMESTAMP |
| toStartOfMonth(toDateOrNull(column)) | перевод str столбца к дате с заполнением пропусков (null) и приводом к началу месяца |
