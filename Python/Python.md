...
> фильтр: выбирает в датасете df все строки, где значение столбца "name" равно "John" и значение столбца "surname" равно "Doe"

`df[df['price']>500]`
> фильтр: выводит строки датасета, где значение столбца "price" больше 500

`df[~(df['price']>500)]`
> фильтр: выводит строки датасета, где значение столбца "price" меньше 500

`df[df['price']>500][['product_id', 'weight']]`
> фильтр: выводит после фильтрации только два указанных столбца в отфильтрованном датасете

`df[df['price'].between(10,20)]`
> фильтр: выводит строки датасета, где значение столбца "price" между 10 и 20 (включительно)<br>
...
> фильтр: выводит строки датасета, в которых по столбцу Title указаны только значения из списка

`df[(df['price']>500)&(df['OrderDate'].between('1997-01-01', '1997-12-31'))]`
> пример объединения нескольких фильтров с оператором and. Объединять можно также с оператором or, который прописывается так: |
