...
| LEAST(num [,num])  | возвращает минимальное значение из списка   | LEAST(1,2,-1) = -1 |
| SIGN([num]) | Возвращает знак переданного числа: -1 для отрицательных значений, 1 для положительных и 0 для 0   | SIGN(-69) = -1,<br> SIGN(0) = 0,<br> SIGN(69) = 1 |
...
| LOG(base, num)      | вычисляет логарифм числа по указанному основанию       | LOG(10, 2) = 0,3010299957 |
| EXP(num)    | вычисляет экспоненциальное значение числа     | EXP(1) = 2,7182818285 |
| SIN(num)   | вычисляет синус числа    | SIN(3) = 0.1411200081 |
| COS(num)   | вычисляет косинус числа    | COS(0.5) = 0.8775825619 |
| TAN(num)   | вычисляет тангенс числа    | TAN(10) = 0.6483608275 |
