...
> statistics, numpy;<br>
> stats from scipy;<br>
> normaltest, shapiro, normaltest, levene, bartlett, norm, ttest_ind, chi2_contingency from scipy.stats;<br>
> math;<br>
> pingouin;<br>
...
###### Распределение Хи-квадрат (Chi-squared) Пирсона

<pre>
<em># Создаем таблицу сопряженности (contingency table)</em>
observed = np.array([
    [15, 20, 25],
    [18, 22, 20],
    [10, 30, 10]
])
<em># Вычисляем статистику Хи-квадрат Пирсона, p-значение, степени свободы и ожидаемые частоты</em>
...
print(expected)
print("\nГипотезы:")
print("H0: переменные независимы (нет связи между переменными)")
print("H1: переменные зависимы (есть связь между переменными)")
print("\nРезультат:")
if p_value < alpha:
    print(f"Отвергаем H0 на уровне значимости {alpha}. Существует статистически значимая связь между переменными.")
else:
    print(f"Не можем отвергнуть H0 на уровне значимости {alpha}. Нет достаточных доказательств связи между переменными.")
</pre>
Вывод:

<pre>
Статистика Хи-квадрат: 10.353280870722731
P-значение: 0.034879154147992864
Степени свободы: 4
Ожидаемые частоты:
[[15.17647059 25.41176471 19.41176471]
 [15.17647059 25.41176471 19.41176471]
 [12.64705882 21.17647059 16.17647059]]
Гипотезы:
H0: переменные независимы (нет связи между переменными)
H1: переменные зависимы (есть связь между переменными)
Результат:
Отвергаем H0 на уровне значимости 0.05. Существует статистически значимая связь между переменными.
</pre>
